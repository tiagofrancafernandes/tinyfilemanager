...
//Site para Encriptar e Decriptar http://www.md5online.org/md5-encrypt.html
...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        if (isset($auth_users[$_POST['fm_usr']]) && md5($_POST['fm_pwd']) === $auth_users[$_POST['fm_usr']]) {
            ...
            fm_set_msg('Login efetuado com sucesso');
            fm_redirect(FM_SELF_URL . '?p=');
        } else {
            unset($_SESSION['logged']);
            fm_set_msg('Senha incorreta', 'error');
            fm_redirect(FM_SELF_URL);
        }
    } else {
        // Form
        ...
        <div class="path login-form">
                <img src="http://cdn.tiagofranca.com/_imagens/favicon.png" alt="H3K File manager" style="margin:20px;">
            <form action="" method="post">
                <label for="fm_usr">Usuário</label><input type="text" id="fm_usr" name="fm_usr" value="" placeholder="Usuário" required><br>
                <label for="fm_pwd">Senha</label><input type="password" id="fm_pwd" name="fm_pwd" value="" placeholder="Senha" required><br>
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if(isset($_POST['type']) && $_POST['type']=="backup") {
        ...
        copy($path.'/'.$file, $path.'/'.$newFile) or die("Não foi possível fazer o backup");
        echo "Backup criado com o nome $newFile";
    }
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if($_GET['type']=="file") {
            if(!file_exists($path . '/' . $new)) {
                ...
                fm_set_msg(sprintf('Arquivo <b>%s</b> criado', fm_enc($new)));
            } else {
                fm_set_msg(sprintf('Arquivo <b>%s</b> já existe', fm_enc($new)), 'alert');
            }
        } else {
            if (fm_mkdir($path . '/' . $new, false) === true) {
                fm_set_msg(sprintf('Pasta <b>%s</b> criada', $new));
            } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                fm_set_msg(sprintf('Pasta <b>%s</b> já existe', fm_enc($new)), 'alert');
            } else {
                fm_set_msg(sprintf('A pasta <b>%s</b> não foi criada', fm_enc($new)), 'error');
            }
        }
    } else {
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                fm_set_msg(sprintf('Movido de <b>%s</b> para <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg('Arquivo ou pasta já existe no destino', 'alert');
            } else {
                fm_set_msg(sprintf('Erro ao mover o arquivo ou pasta <b>%s</b> para <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('Copiado de <b>%s</b> para <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('Erro ao copiar o arquivo ou pasta  <b>%s</b> para <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('As pastas não podem ser iguais', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('Renomeado de  <b>%s</b> para <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('Erro ao renomear arquivo ou pasta de <b>%s</b> para <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nomes não definidos', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        ...
        fm_set_msg('Arquivo não encontrado', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
}
...
if (isset($_POST['upl']) && !FM_READONLY) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('Todos os arquivos carregados para <b>%s</b>', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('Nada carregado', 'alert');
    } else {
        fm_set_msg(sprintf('Erro ao carregar arquivos. Arquivos carregados: %s', $uploads), 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['group'], $_POST['delete']) && !FM_READONLY) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('Os arquivos e pastas selecionados foram excluidos com sucesso');
        } else {
            fm_set_msg('Erro durante a exclusão dos arquivos', 'error');
        }
    } else {
        fm_set_msg('Nada selecionado', 'alert');
    }
...
if (isset($_POST['group'], $_POST['zip']) && !FM_READONLY) {
    ...
    if (!class_exists('ZipArchive')) {
        fm_set_msg('Operações com arquivos indisponível', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if (!empty($files)) {
        ...
        if (count($files) == 1) {
            ...
            $zipname = 'arquivo_zip_' . date('Ymd_His') . '.zip';
        }
        ...
        if ($res) {
            fm_set_msg(sprintf('Arquivo <b>%s</b> criado', fm_enc($zipname)));
        } else {
            fm_set_msg('Arquivo não criado', 'error');
        }
    } else {
        fm_set_msg('Nada selecionado!', 'alert');
    }
...
if (isset($_GET['unzip']) && !FM_READONLY) {
    ...
    if (!class_exists('ZipArchive')) {
        fm_set_msg('Operações com arquivos não estão disponíeis', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
...
if (isset($_POST['copy']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            ?>
            <p class="break-word">Arquivos: <b><?php echo implode('</b>, <b>', $copy_files) ?></b></p>
            <p class="break-word">Pasta de Origem: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
                <label for="inp_copy_to">Pasta de Destino:</label>
                <?php echo FM_ROOT_PATH ?>/<input type="text" name="copy_to" id="inp_copy_to" value="<?php echo fm_enc(FM_PATH) ?>">
            </p>
            <p><label><input type="checkbox" name="move" value="1"> Mover'</label></p>
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> Copiar </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> Cancelar</a></b>
            </p>
        </form>
    </div>
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        <p><b>Copiando</b></p>
        <p class="break-word">
            Pasta de Origem: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Pasta de Destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            Caminho completo: <?php echo fm_enc(fm_convert_win($file_path)) ?><br>
            Tamanho do arquivo: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
           MIME-type: <?php echo $mime_type ?><br>
            ...
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                Arquivos na pasta: <?php echo $total_files ?><br>
                Tamanho total: <?php echo fm_get_filesize($total_uncomp) ?><br>
                Tamanho do arquivo: <?php echo fm_get_filesize($total_comp) ?><br>
        ...
        <p>
            ...
            <b><a href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> Abrir</a></b> &nbsp;
            <?php
            // ZIP actions
            if (!FM_READONLY && $is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> Extrair</a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="UnZip to <?php echo fm_enc($zip_name) ?>"><i class="fa fa-check-circle"></i>
                    Extrair para a pasta</a></b> &nbsp;
                <?php
            }
            if($is_text && !FM_READONLY) {
            ?>
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> Editar</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace" class="edit-file"><i class="fa fa-pencil-square"></i> Edição Avançada</a></b> &nbsp;
            <?php }
            if($send_mail && !FM_READONLY) {
            ?>
            <b><a href="javascript:mailto('<?php echo urlencode(trim(FM_ROOT_PATH.'/'.FM_PATH)) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-pencil-square"></i> Enviar por E-mail</a></b> &nbsp;
            <?php } ?>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-chevron-circle-left"></i> Voltar</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            if ($filenames !== false) {
                ...
                echo '<p>Erro ao buscar informações do arquivo</p>';
            }
        } elseif ($is_image) {
            // Image content
...
if (isset($_GET['edit'])) {
    ...
    if(isset($_POST['savedata'])) {
        ...
        @fwrite($fd, $writedata);
        fclose($fd);
        fm_set_msg('Arquivo salvo om sucesso!', 'alert');
    }
    ...
    <div class="path">
        <div class="edit-file-actions">
            <a title="Cancel" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> Cancelar</a>
            ...
            <?php if($is_text) { ?>
                <?php if($isNormalEditor) { ?>
                    <a title="Editor Avançado" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-paper-plane"></i> Editor Avançado</a>
                    <button type="button" name="Salvar" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Salvar</button>
                <?php } else { ?>
                    <a title="Editor Plano (Simples)" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> Editor Plano (Simples)</a>
                    <button type="button" name="Salvar" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> Salvar</button>
                <?php } ?>
            <?php } ?>
        </div>
        <?php
...
if (isset($_GET['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    ?>
    <div class="path">
        <p><b><?php echo 'Alterar Permissões'; ?></b></p>
        <p>
            <?php echo 'Pasta completa:'; ?> <?php echo $file_path ?><br>
        </p>
        <form action="" method="post">
            <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
            ...
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> Alterar</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> Cancelar</a></b>
            </p>
...
<th>Nome</th><th style="width:10%">Tamanho</th>
<th style="width:12%">Modificado</th>
<?php if (!FM_IS_WIN): ?><th style="width:6%">Permissões</th><th style="width:10%">Proprietário</th><?php endif; ?>
<th style="width:<?php if (!FM_READONLY): ?>13<?php else: ?>6.5<?php endif; ?>%">Ações</th></tr></thead>
<?php
...
<td>Pasta</td><td><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
...
<a title="Excluir" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Excluir Pasta?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
<a title="Renomear" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
<a title="Copiar para..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
<?php endif; ?>
...
<?php if (!FM_READONLY): ?>
<a title="Excluir" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Excluir arquivo?');"><i class="fa fa-trash-o"></i></a>
<a title="Renomear" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
<a title="Copiar para..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
<?php endif; ?>
<a title="Link direto" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
<a title="Baixar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
<?php if (!FM_READONLY): ?>
<p class="path footer-links"><a href="#/select-all" class="group-btn" onclick="select_all();return false;"><i class="fa fa-check-square"></i> Selecionar todos</a> &nbsp;
<a href="#/unselect-all" class="group-btn" onclick="unselect_all();return false;"><i class="fa fa-window-close"></i> Desmarcar todos</a> &nbsp;
<a href="#/invert-all" class="group-btn" onclick="invert_all();return false;"><i class="fa fa-th-list"></i> Inverter seleção</a> &nbsp;
<input type="submit" class="hidden" name="delete" id="a-delete" value="Delete" onclick="return confirm('Excluir pastas e arquivos selecionados?')">
<a href="javascript:document.getElementById('a-delete').click();" class="group-btn"><i class="fa fa-trash"></i> Excluir </a> &nbsp;
<input type="submit" class="hidden" name="zip" id="a-zip" value="Zip" onclick="return confirm('Criar Arquivo?')">
<a href="javascript:document.getElementById('a-zip').click();" class="group-btn"><i class="fa fa-file-archive-o"></i> Comprimir em ZIP </a> &nbsp;
<input type="submit" class="hidden" name="copy" id="a-copy" value="Copy">
<a href="javascript:document.getElementById('a-copy').click();" class="group-btn"><i class="fa fa-files-o"></i> Copiar </a>
<a href="https://neonservicos.com.br" target="_blank" class="float-right" style="color:silver">NeOn Gerenciador de Arquivos</a></p>
<?php endif; ?>
</form>
...
<div class="path main-nav">
        ...
        <?php if (!FM_READONLY): ?>
        <a title="Pesquisar na pasta atual" href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-search"></i></a>
        <a title="Upload de Arquivos" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i></a>
        <a title="Criar novo item" href="#createNewItem" ><i class="fa fa-plus-square"></i></a>
        <?php endif; ?>
        <?php if (FM_USE_AUTH): ?><a title="Sair (Fazer Logoff)" href="?logout=1"><i class="fa fa-sign-out" aria-hidden="true"></i></a><?php endif; ?>
        </div>
</div>
<?php
}
...
<head>
<meta charset="utf-8">
<title>NeOn Gerenciador de Arquivos 1</title>
   <script src="emmet/emmet.js"></script>
<script>
...
<head>
<meta charset="utf-8">
<title>NeOn Gerenciador de Arquivos 2</title>
<meta name="Description" CONTENT="Author: CCP Programmers, H3K Tiny PHP File Manager">
...
<body>
<div id="wrapper">
  <div id="createNewItem" class="modalDialog"><div class="model-wrapper"><a href="#close" title="Close" class="close">X Fechar</a><h2>Criar Novo Item</h2><p>
        <label for="newfile">Tipo do Item &nbsp; : </label><input type="radio" name="newfile" id="newfile" value="file">Arquivo <input type="radio" name="newfile" value="folder" checked> Pasta<br><label for="newfilename">Nome do Item: </label><input type="text" name="newfilename" id="newfilename" value="" placeholder="Nome do arquivo" required><br>
        <input type="submit" name="submit" class="group-btn" value="Criar agora" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"></p></div></div>
    <div id="searchResult" class="modalDialog"><div class="model-wrapper"><a href="#close" title="Close" class="close">X Fechar</a>
    <input type="search" name="search" value="" placeholder="Encontre um item na pasta atual...">
